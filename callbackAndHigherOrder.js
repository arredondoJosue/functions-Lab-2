...
uniq(names, callback2 = (uniqArr) => {console.log(`The new names array with all the duplicate items removed is ${uniqArr}`);})
...
each(names, callback2 = (item, index) => {console.log(`The item at index ${index} is ${item}.`)})
...
// const getUserById = users.filter(users => users.id === id)
// console.log(getUserById);

const getUserById = (arrObj, id, callback) => arrObj.forEach(users => users.id === id ? callback(users) : null)
